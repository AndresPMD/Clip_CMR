...
from bias_utils import SentenceSimplifier, updateCovMatrix
import re
import random
import itertools
import collections
...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, opt):
        ...
        self.vocab = vocab
        self.loc = data_path + '/'
        self.data_name = data_path.split('/')[-1].split("_")[0]
        self.seq_per_img = 5
        self.categories = opt.categories

        #
        self.augmentation = opt.augmentation
        self.aug_caption = opt.aug_caption
        ...
        self.class2id = jsonmod.load(open('/home/amafla/Documents/Python_Codes/Bias_in_CMR/object_names.json', 'rb'))
        self.id2class = {v: k for k, v in self.class2id.items()}
        # Load category ids
        with open("/home/amafla/Documents/Python_Codes/Bias_in_CMR/fasttext_rep.json", "r") as fp:
            self.fasttext_object_ids = jsonmod.load(fp)

        # LOAD OBJECT_IDS
        with open("/home/amafla/Documents/Python_Codes/Bias_in_CMR/object_ids_%s.json" % self.data_name, "r") as fp:
            self.obj_ids_dict = jsonmod.load(fp)

        with open("/home/amafla/Documents/Python_Codes/Bias_in_CMR/%s_idx2image_f30k.json" % data_split, 'r') as fp:
            self.index2file = jsonmod.load(fp)
        self.file2index = {v: k for k, v in self.index2file.items()}

        # Cov Matrix
        with open("/home/amafla/Documents/Python_Codes/Bias_in_CMR/covMatrix/covMatrixAnns_%s.json" % self.data_name, "r") as fp:
            self.covMatrix = np.array(jsonmod.load(fp))

        ord_dict = collections.OrderedDict(sorted(self.id2class.items()))
        covMatrix_map = list(v for k, v in ord_dict.iteritems())

        self.covMatrix_id2class = {idx: val for idx, val in enumerate(covMatrix_map)}
        self.covMatrix_class2id = {v: k for k, v in self.covMatrix_id2class.items()}
    ...
    def __getitem__(self, index):
        ...
        if self.data_name == 'f30k':
            if self.aug_caption =='debias' and self.data_split == 'train':
                cap_ids, obj_ids, wordToReplace, replacement_word, pair, replaced_caption = self.augment(img_id, caption)
            else:
                # Aug caption strategy not implemented...yet (bias)
                replaced_caption = False

        else:
            print("Coco yet to be implemented")


        if replaced_caption is not False and self.data_split == 'train':
            caption = replaced_caption
            obj_names = [self.id2class[i] for i in obj_ids]
            obj_names = [replacement_word if i == wordToReplace else i for i in obj_names]
        else:
            f30k_img_id = self.index2file[str(img_id)]
            obj_ids = self.obj_ids_dict[f30k_img_id.strip()]
            obj_names = [self.id2class[i] for i in obj_ids]

        # Get the fasttext of object names
        categories = np.zeros((self.categories, 300))
        for idx, elem in enumerate(obj_names):
            if idx < self.categories:
                categories[idx] = self.fasttext_object_ids[elem]
            else: continue
        ...
        return image, target, index, img_id, caption_label, caption_mask, categories
    ...
    def inv_multinomial(self, pair):
        cls_idx = self.covMatrix_class2id[pair[0]]
        ...
        # replacement_word = self.id2class[np.argmax(np.random.multinomial(1, class_probabilities))]
        replacement_word = self.covMatrix_id2class[np.argmax(np.random.multinomial(1, class_probabilities))]
        return replacement_word

    def replace_words(self, synonym_pair, replacement_word, simplified_string):
        simplified_string = re.sub(r"\b{}\b".format(synonym_pair[1]), " {} ".format(replacement_word), simplified_string)
        simplified_string = re.sub(r" nt", "nt", simplified_string)
        simplified_string = simplified_string.strip().replace("  "," ")
        simplified_string = re.sub(r'[^A-Za-z ]+', '', simplified_string)

        # cap_ids = [int(self.word_to_ix[i]) for i in simplified_string.lower().split()]
        cap_ids = []
        stringer = simplified_string.lower().split()
        for i in stringer:
            if i in self.vocab.word2idx.keys():
                cap_ids.append(int(self.vocab.word2idx[i]))
            # if i in self.word_to_ix.keys():
            #     cap_ids.append(int(self.word_to_ix[i]))

        return cap_ids, simplified_string

    def augment(self, img_id, caption_string):
        # Ids depending on dataset
        if self.data_name == 'coco':
            coco_img_id = self.img_id_file[img_id]
            obj_ids = self.obj_ids_dict[coco_img_id.strip()]
        else:
            f30k_img_id = self.index2file[str(img_id)]
            obj_ids = self.obj_ids_dict[f30k_img_id.strip()]
        ...
        if isinstance(caption_string, str):
            caption_string = unicode(caption_string, "utf-8")
        ...
        prob_replacement = np.random.rand()
        if prob_replacement < 0.5 and pair[0] in self.class2id.keys() and pair[1] in self.class2id.keys():
            ...
            while replacement_word == pair[0] or replacement_word == wordToReplace:
                if self.augmentation == 'uniform':
                    ...
                    replacement_word = self.inv_multinomial(pair)
                    #cls_pair = (self.class2id[wordToReplace], self.class2id[replacement_word])
                    cls_pair = (self.covMatrix_class2id[wordToReplace], self.covMatrix_class2id[replacement_word])
                    self.covMatrix = updateCovMatrix(self.covMatrix, set(obj_names), cls_pair, self.covMatrix_class2id)

            # Using simplified caption
            cap_ids, replaced_caption = self.replace_words(synonym_pair, replacement_word, simplified_string)
            ...
            return cap_ids, obj_ids, wordToReplace, replacement_word, pair, replaced_caption
        else:
